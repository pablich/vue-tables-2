 ...
 module.exports = function(h, that) {
    ...
    var search = that.source=='client'?
                ...
                var loadTag = function loadTag(e) {
                  ...
                  words.forEach(function (element) {
                    if(element !== ' ' && element !== '' && !checkIfExist(element))
                    container.insertAdjacentHTML('beforeend',
                    '<span class="tag" style="margin-top: 5px;margin-left: 10px;">' + 
                    '<span class="has-ellipsis" style="padding-right: 4px">' + element + '</span>' + 
                    '<a class="delete is-small"></a>' + '</span>');
                  });
  ...
  if (that.opts.filterable && !that.opts.filterByColumn) {
      ...
      return <div class="field is-horizontal is-pulled-left VueTables__search">
      {beforeFilter}
      <input class="input filter-search"
      type="text"
      value={that.query}
      placeholder={that.display('filterPlaceholder')}
      on-blur={debounce(loadTag, that.opts.debounce)}
      id={id}
      style="width:180px"
      />
